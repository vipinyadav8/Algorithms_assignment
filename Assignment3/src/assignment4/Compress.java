...
public class Compress {
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            inputStream = new FileReader(originalFilename);
            outputStream = new FileWriter(compressedFilename);
            ...
            while((q = inputStream.read()) != -1) {
                ...
                if(dict.containsKey(p+c)){
                    ...
                    //byte[] code1 = intToByte(dict.get(p));
                    outputStream.write(dict.get(p) + " ");
                    //outputStream.write(code1[1] + " ");
                    dict.put(p+c, count++);
                    p = c;
                }
            }
            //byte[] code2 = intToByte(dict.get(p));
            //outputStream.write(code2[0] + "");
            outputStream.write(dict.get(p) + "");
        } finally {
            if (inputStream != null) {
                inputStream.close();
