...
//        root = new BinNode(null, null, null, null, null, null);
    ...
    public BinNode search(BinNode x, int i, int b){
        if(x != null){
            if(x.getBinCapacity() < b){
                return search(x.getRightChild(), i, b);
            }
            else if(x.getBinCapacity() > b){
                return search(x.getLeftChild(), i, b);
            }
            else{
                if(i != x.getBinId()){
                    return search(x.getRightChild(), i, b);
    ...
    public void insertBin(BinNode z){
        ...
        if(this.root() == null){
            ...
            while(x != null){
                y = x;
                if(x.getBinCapacity() <= z.getBinCapacity()){                    
                    x = x.getRightChild();
                }
                else{                    
                    x = x.getLeftChild();
            ...
            if(y.getBinCapacity() <= z.getBinCapacity()){                
                y.setRightChild(z);
            }
            else{
                y.setLeftChild(z);
    ...
    public BinNode deleteBin(int i, int c){     
        //System.out.println("delete method is called for bin capacity c : " + c);
        BinNode x = search(this.root(), i, c);
        //System.out.println("delete method is called for x : " + x);
        BinNode temp = x;
        BinNode y;
        BinNode z;        
        BinNode w;
        if(x.getLeftChild() == null && x.getRightChild() == null){            
            ...
            if(x == root && x != y.getParent()){
                temp1 = deleteBin(y.getBinId(),y.getBinCapacity());            
                replaceRoot(temp1, x);
                root = y;
                check(w);
            }
